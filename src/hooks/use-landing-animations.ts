...
import { formatStat, prefersReducedMotion } from '../lib/helpers';
 ...
 * - `data-cells` — grid-cell stagger; `data-revealed` is set on the grid (not each cell)
 *   so dynamic children remain visible after remount
 * - `data-counter` / `data-counter-format` — animated stat counters
 *
 * Reveal initial state lives in CSS (see globals.css) — never `gsap.set` before
 * hydration. Eager inline styles race Next.js Suspense/dynamic islands
 * (InTheField, ChamberMap) and cause hydration mismatches.
 */
export function useLandingAnimations(rootRef: RefObject<HTMLDivElement | null>) {
  useEffect(() => {
    ...
    // Double rAF: wait until after paint so Suspense/dynamic children have hydrated.
    outerFrame = requestAnimationFrame(() => {
      innerFrame = requestAnimationFrame(() => {
        ...
        ctx = gsap.context(() => {
          ...
          const markRevealed = (els: Element | Element[]) => {
            const list = Array.isArray(els) ? els : [els];
            list.forEach((el) => el.setAttribute('data-revealed', ''));
          };
